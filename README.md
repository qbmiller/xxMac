...
- The clipboard database, original image cache, and thumbnail cache are stored as `clipboard.db`, `clipboard_images/`, and `clipboard_thumbnails/` inside the configuration folder. Clipboard favorite and favorite-pin state is stored in `clipboard.db`; removing an item from Favorites keeps the underlying history record and clears its favorite pin.
