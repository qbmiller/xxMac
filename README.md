...
| Todo | Open an independent resizable window for local tasks. It includes a dedicated 2 x 2 Eisenhower matrix, All/Today/status views, card and list layouts, and an expandable three-column Todo/In Progress/Completed board. Tasks support title, notes, quadrant, minute-precision deadline, search, editing, archive/permanent delete, status rollback, and drag-and-drop ordering across quadrants or status columns. Completed tasks stay visible in their quadrant with strikethrough styling. | Microsoft To Do / TickTick / Trello |
| Todo Desktop Widget | On macOS 14 or later, add the medium xxMac Todo widget from the desktop's Edit Widgets gallery. It shows up to 10 incomplete, unarchived tasks in one column, ordered by Today (overdue and due today), In Progress, then Todo. Complete a task directly from the widget; clicking elsewhere opens the Todo window. Use Show Desktop, including F11 when configured for that action, to view it on the desktop. | WidgetKit / TickTick Widget |
...
- Todo tasks are stored locally in `todo.db` inside the configuration folder. The database and its SQLite sidecar files move when the configuration folder changes. Regenerable widget snapshots and pending completion actions stay in the system-managed App Group container and do not move with the configuration folder. Todo does not implement iCloud or CloudKit sync.
