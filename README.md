...
xxMac 是一个安装包约 2 MB 的轻量级 macOS 原生菜单栏效率工具，基于 `SwiftUI + AppKit` 构建，支持窗口管理、全局快捷键、启动器、中国日历、快捷键冲突定位、剪贴板历史与常用工作流。它把日常高频操作整合到一个轻量入口里，日常使用形态是：
...
| 中国日历 | 提供菜单栏日历图标入口，支持中国农历、节假日、节气、周数和菜单栏图标样式配置。 | CalendarX |
