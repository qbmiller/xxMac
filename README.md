...
| Clipboard History [disabled by default] | Records text and image clipboard items, persists them with SQLite, and supports search, preview, and paste-back. The image Toggle writes `img` into the search field to show all images. Large text previews show only the first part while paste-back keeps the full content; images above the configured threshold get thumbnail previews; local OCR is enabled by default and stores recognized image text as searchable metadata. | Clipboard manager |
