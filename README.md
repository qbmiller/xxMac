...
| Finder Paste Operations | Press `Command + Shift + V` to paste paths for files or folders copied in Finder. With the optional Image to File or Text to File switches enabled under Clipboard > Paste Operations, the same hotkey in Finder saves a raw clipboard image as PNG or plain text as a UTF-8 file in the current folder. Copied files always remain path-paste input and are never converted. | Copy Path / Path Finder |
