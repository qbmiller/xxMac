...
| Todo | Open an independent resizable window for local tasks. It includes a dedicated 2 x 2 Eisenhower matrix, fixed All/Today/status views, user-defined lists in the sidebar, card and list layouts, and an expandable three-column Todo/In Progress/Completed board. Lists can be created, renamed, and deleted; deleting a list keeps its tasks and makes them unassigned. Tasks support title, notes, list selection, direct four-color quadrant selection, minute-precision deadline, search, editing, archive/permanent delete, status rollback, and drag-and-drop ordering across quadrants or status columns. Task title font size is configurable under Settings > Todo > General. Completed tasks stay visible in their quadrant with strikethrough styling. | Microsoft To Do / TickTick / Trello |
