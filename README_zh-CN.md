...
| 待办 | 通过独立且可调节大小的窗口管理本地任务。包含独立 2 x 2 四象限、“全部 / 今日 / 待办 / 进行中 / 已完成”视图、卡片与列表布局，以及可展开的三栏状态看板。任务支持标题、备注、四象限、精确到分钟的截止时间、搜索、编辑、归档/永久删除、状态撤回，并可在象限或状态栏内及跨栏拖动排序；任务标题字体大小可在“设置 > 待办 > 通用”中配置。已完成任务仍保留在原象限，并以划线样式显示。 | Microsoft To Do / 滴答清单 / Trello |
...
- 待办任务保存在配置目录的本地 `todo.db` 中；修改配置目录时会连同 SQLite 辅助文件一起迁移。可重新生成的小组件快照、小组件显示设置、分页状态和待确认完成操作保存在固定的 `~/Library/Application Support/xxMac/Widget/` 中，不随配置目录迁移。Widget 的沙盒权限仅开放该专用目录。待办不支持 iCloud 或 CloudKit 同步。
