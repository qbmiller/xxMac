...
| Todo 桌面小组件 | macOS 14 及以上可从桌面“编辑小组件”添加 xxMac Todo 中号组件。组件以单列显示最多 10 条未完成且未归档任务，按“今日（逾期和今天到期）→ 进行中 → 待办”排序；可直接点击圆圈完成任务，点击其他区域打开 Todo 窗口。使用“显示桌面”（系统配置为 F11 时可按 F11）即可在桌面查看。 | WidgetKit / 滴答清单小组件 |
...
- 待办任务保存在配置目录的本地 `todo.db` 中；修改配置目录时会连同 SQLite 辅助文件一起迁移。可重新生成的小组件快照和待确认完成操作保存在系统管理的 App Group 容器中，不随配置目录迁移。待办不支持 iCloud 或 CloudKit 同步。
