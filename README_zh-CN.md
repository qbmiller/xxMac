...
| 剪贴板历史 [默认关闭]| 记录文本和图片剪贴板，使用 SQLite 持久化，支持检索、预览、回贴、收藏和收藏置顶；面板默认打开“历史”，可用 Tab / Shift+Tab 在“历史 / 历史-图片 / 收藏 / Snippets”之间切换。选择“历史-图片”会在搜索栏显示 `img`，并且只展示图片。上下选择图片后按空格可在应用自管的屏幕级预览中放大查看原图，支持触控板捏合缩放和平移；1 倍时拖动图片可把预览移到屏幕任意位置，放大后拖动或双指滚动可查看图片局部。再按空格或 Esc 只关闭图片预览，剪贴板面板保持打开。Command+Return 可收藏历史选中项，或从收藏中移出选中项且保留原历史记录；收藏页右侧红色星星负责取消收藏，pin 按钮负责在收藏列表内置顶/取消置顶。收藏后的记录不会被自动清理或手动清空历史删除，从收藏中移除时会保留原历史记录。大文本预览只显示前一部分但回贴保留完整内容；图片项会显示宽高和大小，超过阈值时生成缩略图用于预览；可选启用本地 OCR，把图片文字作为 metadata 写入数据库用于搜索。可通过自定义全局热键或菜单栏“剪贴板历史”打开，密码输入框等安全输入场景下会使用高层级浮层显示。 | 剪贴板管理器 |
...
| 更新检查 | 可在“关于”中手动检查，也可选择关闭、每天、每周或每月自动检查，默认每周。自动检查不会弹窗；发现新版本后，启动器右侧显示红色更新按钮，点击打开 GitHub Releases。 | Sparkle |
...
- 热键配置、应用快捷启动配置、启动器窗口宽高、整体大小、文字大小与外观、启动器最近操作历史、语言偏好、快捷指令、浏览器搜索、Snippets、日历偏好和更新检查状态保存在配置目录的 `preferences.json`。更新检查会保存 `UpdateCheckFrequency`、`UpdateLastSuccessfulCheck` 和 `UpdateAvailableVersion`，首次使用默认每周检查。启动器最近操作历史只记录应用、窗口命令、快捷指令和计算器结果的元数据，不记录剪贴板历史或 Snippets 内容；默认最多保留 100 条，可在“搜索 > 通用”配置。日历偏好包含状态栏显示方式，默认使用日历图标，也可切换为 App 图标。首次启动默认值集中在 `Sources/xxMac/AppDefaultSettings.swift`，该文件可用注释说明默认开关。
...
- 剪贴板数据库、图片原图缓存与图片缩略图缓存位于配置目录的 `clipboard.db`、`clipboard_images/` 和 `clipboard_thumbnails/`；剪贴板收藏状态和收藏置顶状态也保存在 `clipboard.db`，从收藏中移除只取消收藏并清掉收藏置顶，不删除原历史记录。
