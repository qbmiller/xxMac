...
- When Clipboard History or Favorites has an image selected, pressing Command+Space
...
- Pressing Command+Space or Escape closes the image preview.
...
- Route Command+Space and Escape through the existing launcher-level keyboard handler
...
- Manually verify Command+Space opens the original image above the launcher, the preview
  can move across the screen, pinch and pan work, and Command+Space/Escape close only
  the preview.
